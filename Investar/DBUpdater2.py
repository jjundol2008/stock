...
def replace_into_index(df, num, code, company):
        """네이버에서 읽어온 주식 시세를 DB에 REPLACE """
        index = 'daily_price'
        es = Elasticsearch(['192.168.0.13','192.168.0.14','192.168.0.15'], port=9200)
...
def update_daily_price(sub_codes, pages_to_fetch, code_list_number, result):
    """KRX 상장법인의 주식 시세를 네이버로부터 읽어서 DB에 업데이트"""
    print('pages_to_fetch', pages_to_fetch)
    print(f'child no[{code_list_number}]')

       
    for idx, code in enumerate(sub_codes):
        df = read_naver(code, sub_codes[code], pages_to_fetch)
        if df is None:
            continue
        replace_into_index(df, idx, code, sub_codes[code])
    
    
    result.put(code_list_number)
...
if __name__ == '__main__':
    ...
    sub_codes_list = dbu.split_dict_equally(dbu.codes, 10)
